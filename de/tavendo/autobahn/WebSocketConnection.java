...
public class WebSocketConnection implements WebSocket {
	...
	WebSocketConnection.SocketThread e = this.mSocketThread;
	...
	if(this.mSocket == null) {
		...
		this.createReader();
		this.createWriter();
		ClientHandshake e1 = new ClientHandshake(this.mWebSocketURI, (URI)null, this.mWebSocketSubprotocols);
	...
	if(message.obj instanceof TextMessage) {
	    TextMessage error = (TextMessage)message.obj;
	    if(webSocketObserver != null) {
	    ...
	    RawTextMessage error1 = (RawTextMessage)message.obj;
	    if(webSocketObserver != null) {
	    ...
	    BinaryMessage error2 = (BinaryMessage)message.obj;
	    if(webSocketObserver != null) {
	    ...
	    ServerHandshake error7 = (ServerHandshake)message.obj;
	    ...
	    Error error6 = (Error)message.obj;
	    this.failConnection(WebSocketCloseNotification.INTERNAL_ERROR, "WebSockets internal error (" + error6.mException.toString() + ")");
	} else if(message.obj instanceof ServerError) {
	    ServerError error8 = (ServerError)message.obj;
	    this.failConnection(WebSocketCloseNotification.SERVER_ERROR, "Server error " + error8.mStatusCode + " (" + error8.mStatusMessage + ")");
	} else {
